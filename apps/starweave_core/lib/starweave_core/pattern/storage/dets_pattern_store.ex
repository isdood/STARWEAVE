defmodule StarweaveCore.Pattern.Storage.DetsPatternStore do
  ...
  def put(id, pattern_data) when is_binary(id) and is_map(pattern_data) do
    case :dets.insert(@dets_table, {id, pattern_data}) do
      :ok -> :ok
      error ->
  ...
  def get(id) when is_binary(id) do
    case :dets.lookup(@dets_table, id) do
      [{^id, pattern_data}] -> {:ok, pattern_data}
      [] -> :not_found
      error ->
  ...
  or `{:error, reason}` on failure.
  """
  @spec all() :: {:ok, [{pattern_id(), pattern_data()}]} | {:error, term()}
  def all do
    case :dets.match_object(@dets_table, :_) do
      patterns when is_list(patterns) ->
        {:ok, patterns}
      error ->
        Logger.error("Error retrieving all patterns: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  or `{:error, reason}` on failure.
  """
  @spec delete(pattern_id()) :: :ok | :not_found | {:error, term()}
  def delete(id) when is_binary(id) do
    case :dets.member(@dets_table, id) do
      true ->
        case :dets.delete(@dets_table, id) do
          :ok -> :ok
          error ->
            Logger.error("Error deleting pattern #{id}: #{inspect(error)}")
            error
        end
      false ->
        :not_found
      error ->
        Logger.error("Error checking pattern existence for #{id}: #{inspect(error)}")
        {:error, error}
    end
  end
