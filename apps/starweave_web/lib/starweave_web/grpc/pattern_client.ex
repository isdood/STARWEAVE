defmodule StarweaveWeb.GRPC.PatternClient do
  ...
  def analyze_pattern(%{} = pattern, opts \\ []) do
    with {:ok, request} <- build_pattern_request(pattern, opts),
      ...
      result = PatternServiceStub.recognize_pattern(channel, request, timeout: channel_opts[:timeout])
  ...
  defp build_channel_opts(opts) do
    ...
    connect_timeout = Keyword.get(opts, :connect_timeout, Keyword.get(config, :connect_timeout, @default_connect_timeout))
  ...
  defp parse_endpoint(endpoint) when is_binary(endpoint) do
    ...
    case String.split(endpoint, ":") do
      [host] -> 
        {host, 50051}  # Default gRPC port if not specified
      [host, port_str] -> 
  ...
  """
  @spec recognize_pattern(map(), [String.t()] | nil) :: {:ok, PatternResponse.t()} | {:error, any()}
